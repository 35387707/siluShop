...
namespace silushop.Utils
{
    public class WX_Services
    {
        ...
        public class WxPayData
        {
            public WxPayData()
            {
                if (wxAppid==null) {
                    ...
                    wxAppid = Convert.ToString(SysConfigBLL.SysConfigList.FirstOrDefault(m => m.Name == "WXAPPID").Value);
                    wxMCHID = Convert.ToString(SysConfigBLL.SysConfigList.FirstOrDefault(m => m.Name == "WXMCHID").Value);
                    wxKey = Convert.ToString(SysConfigBLL.SysConfigList.FirstOrDefault(m => m.Name == "WXKEY").Value);
                    wxPayKey = Convert.ToString(SysConfigBLL.SysConfigList.FirstOrDefault(m => m.Name == "WXKEY").Value);
                }
            }
            ...
            /// 商户号
            /// </summary>
            public static string wxMCHID = null;
            /// <summary>
            /// 公众号秘钥 仅JSAPI支付的时候需要配置
            /// </summary>
            public static string wxKey = null;
            /// <summary>
            /// 公众号支付秘钥 
            /// </summary>
        ...
        public class WxPayApi
        {
            ...
            /// 统一下单
            /// </summary>
            /// <param name="body">商品描述String(128) 例：腾讯充值中心-QQ会员充值</param>
            /// <param name="out_trade_no">商户订单</param>
            /// <param name="total_fee">订单总金额，单位为分</param>
            /// <param name="spbill_create_ip">用户端ip</param>
            public string UnifiedOrder(string body, string out_trade_no, int total_fee, string spbill_create_ip, string notify_url, out WxPayData dataout)
            {
                string url = "https://api.mch.weixin.qq.com/pay/unifiedorder";

                WxPayData data = new WxPayData();
                data._encrypt = euEncryptType.MD5;
                data.SetValue("appid", WxPayData.wxAppid);//公众账号id
                data.SetValue("body", body);//商品描述
                data.SetValue("mch_id", WxPayData.wxMCHID);//商户号
                data.SetValue("nonce_str", getString());//随机字符串
                string nonce_str = getString();//随机字符串
                data.SetValue("notify_url", notify_url);//回掉地址
                data.SetValue("out_trade_no", out_trade_no);//商户订单
                ...
                data.SetValue("sign", data.MakeSign(WxPayData.wxPayKey));
                dataout = data;
                return HttpService.Post(data.ToXml(), url, false, 20);
            }
            ...
            public string GetWXPCLoginURL(string redirect_uri, euScope scope, string param)
            {
                string url = "https://open.weixin.qq.com/connect/qrconnect?appid={0}&redirect_uri={1}&response_type=code&scope={2}&state={3}#wechat_redirect";
                return string.Format(url, WxPayData.wxAppid, HttpContext.Current.Server.UrlEncode(redirect_uri), scope, param);
                //如果用户同意授权，页面将跳转至 redirect_uri/?code=CODE&state=STATE。
                //若用户禁止授权，则重定向后不会带上code参数，仅会带上state参数redirect_uri?state=STATE 
            }
                    ...
                    if (dic != null && dic.Keys.Contains("openid"))
                    {
                        ...
                        if (string.Compare(dic["scope"].ToString(), ((int)euScope.snsapi_userinfo).ToString(), true) == 1)
                        {
                            ...
                            if (dic != null && dic.Keys.Contains("openid"))
                            {
                                ...
                                user.unionid = dic["unionid"].ToString();
                            }
                        }
                    }
                }
             ...
             */
            public WxPayData GetUnifiedOrderResult(int total_fee, string order_desc, string openid, string orderid, string notify_url)
            {
                //统一下单
            ...
            */
            private WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
            {
                ...
                //inputObj.SetValue("spbill_create_ip", "119.23.69.119");//终端ip
                ...
                //logbll.InsertLog("WX_xml=" + xml, Common.enLogType.DEBUG);
